...
def set_debug(debug=True, filename='dicttoxml.log'):
    if debug:
        import datetime
        print
        'Debug mode is on. Events are logged at: %s' % filename
...
def unicode_me(something):
    ...
    for `str()`, but `str()` doesn't take a second argument,
    hence this kludge."""
    try:
...
def make_attrstring(attr):
    ...
    attrstring = ' '.join(['%s="%s"' % (k, v) for (k, v) in
                           attr.items()])
...
def key_is_valid_xml(key):
    ...
    LOG.info('Inside key_is_valid_xml(). Testing "%s"'
             % unicode_me(key))
    test_xml = '<?xml version="1.0" encoding="UTF-8" ?><%s>foo</%s>' \
               % (key, key)
    try:
...
def convert_dict(
    ...
    for (key, val) in obj.items():
        LOG.info('Looping inside convert_dict(): '
                 'key="%s", val="%s", type(val)="%s"'
                 % (unicode_me(key), unicode_me(val),
                    type(val).__name__))
        ...
        if isinstance(val, numbers.Number) or type(val) in (str,
                                                            unicode):
            ...
            addline('<%s%s>%s</%s>' % (key, make_attrstring(attr),
                                       convert_dict(val, ids, key, attr_type), key))  # noqa
            ...
            raise TypeError('Unsupported data' /
                            ' type: %s (%s)' % (val, type(val).__name__))
    return ''.join(output)
...
def convert_kv(
    ...
    return '<%s%s>%s</%s>' % (key, attrstring, xml_escape(val), key)
...
def convert_bool(
    ...
    if attr_type:
        attr['type'] = get_xml_type(val)
    attrstring = make_attrstring(attr)
    return '<%s%s>%s</%s>' % (key, attrstring, unicode(val).lower(),
                              key)
...
def dicttoxml(
    ...
    attr_type is used to specify if data type for each
    element should be included in the resulting xml.
